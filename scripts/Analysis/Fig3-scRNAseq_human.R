...
data <- Read10X(data.dir = "~/Dropbox (MN)/Per/PhD/Projects/Chimp/20180521_10xInhouse_scRNAseq/matrices/hiPS6.d14_filtered_gene_bc_matrices/GRCh38/")
...
## run TSNE
data <- RunTSNE(object = data, dims.use = 1:10, do.fast = TRUE,dim.embed = 3)
...
filename <- paste(plotDir,"Fig3_human_pie_neuroD1.G1.pdf",sep = "")
...
filename <- paste(plotDir,"Fig3_human_pie_ANKRD1.CTGF.pdf",sep = "")
...
filename <- paste(plotDir,"Fig3D_human_tSNE_NeuroD1_G1_tsne1.tsne2.pdf",sep = "")
...
filename <- paste(plotDir,"Fig3D_human_tSNE_ANKRD1.CTGF_tsne1.tsne2.pdf",sep = "")
pdf(filename,width = 6,height = 6)
par(mfrow=c(1,1))
ggplot(data = tsne.d123,mapping = aes(x = tSNE_1,y = tSNE_2)) +
  geom_point(col="black",alpha=.1,size=.7) +
...
filename <- paste(plotDir,"Fig3D_human_tSNE_black_tsne1.tsne2.pdf",sep = "")
pdf(filename,width = 6,height = 6)
par(mfrow=c(1,1))
ggplot(data = tsne.d123,mapping = aes(x = tSNE_1,y = tSNE_2)) +
...
head(tsne.d123)
filename <- paste(plotDir,"Fig3C_human_tSNE_foxg1.pax6.otx2_tsne1.tsne2.pdf",sep = "")
...
filename <- paste(plotDir,"Fig3A_human_PCA_CCcolors.pdf",sep = "")
pdf(file = filename)
...
pdf(file = paste(plotDir,"Fig3B_human_PCA_varGenes.regressedCC_colorCC.pdf",sep=""))
