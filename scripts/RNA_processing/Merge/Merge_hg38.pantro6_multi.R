...
path <- "~/Dropbox (MN)/Per/PhD/Projects/KRABZNF-manuscript/Data/bulkRNA_allQuant/hg38.s2.multi.Gencode27.Exon.sjdb.txt"
...
colNames <- gsub(pattern ="X.projects.fs1.medpvb.backup.projects.ChimpHuman.RNAseq.Aligned_hg38_STAR_mMap.sjdb.genc.v27.hg38.mMap.genc.v27.",replacement = "",gsub(pattern = "Aligned.out.bam",replacement = "",x = names(header)))
...
colDat.hs <- colDat
colDat.hs$mapGenome <- "hg38"
colDat.hs$mapType <- "multiMap"
...
## FeatureCount raw file
path <- "~/Dropbox (MN)/Per/PhD/Projects/KRABZNF-manuscript/Data/bulkRNA_allQuant/pantro6.s2.multi.Gencode27.Exon.sjdb.txt"

## Get the sampleFile name header
header <- read.delim(path,nrows = 1,skip=1,header=T)
## Define sample names, by removing pre- and suffix 
colNames <- gsub(pattern ="X.projects.fs1.medpvb.backup.projects.ChimpHuman.RNAseq.Aligned_pantro6_STAR_mMap.sjdb.pantro6.mMap.sjdb.genc27.lifted.",replacement = "",gsub(pattern = "Aligned.out.bam",replacement = "",x = names(header)))
...
colDat <- data.frame(species=species,time=time,condition=species,line=line,samples=colNames[-c(1:6)],batch=batch)
colDat.pt <- colDat
colDat.pt$mapGenome <- "pantro6"
colDat.pt$mapType <- "multiMap"
...
path.merged.PC <- "merge/Merged_MultiMapping_Hg38.PanTro6_ProteinCoding.txt"
...
## All genes
header <- read.delim(file = path,header = F,nrows = 1)
path.merged <- "merge/Merged_MultiMapping_Hg38.PanTro6_allGenes.txt"
...
chimp.coldat$mapping <- "multiMap"
human.coldat <- colDat[colDat$species=="human",]
human.coldat$mapGenome <- "hg38"
human.coldat$mapping <- "multiMap"

write.table(x = rbind(human.coldat,chimp.coldat),file = "Merge/Merged_MultiMapping_Hg38.PanTro6_ColData.txt",quote = F,row.names = F,col.names = T,sep = "\t")

write.table(x = colDat,file = "ColDat_default.txt",quote = F,row.names = F,col.names = T,sep = "\t")
